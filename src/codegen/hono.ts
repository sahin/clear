...
export function generateHonoCode(ast: ClearFile): string {
  const parts: string[] = []

  parts.push(`// Auto-generated by Clear v0.2 — Hono`)
  parts.push(`// Source: ${ast.product.name}`)
  parts.push(`// Do not edit — changes should be made to the .clear file`)
  parts.push('')

  const imports = new ImportMap()
  imports.add('Hono', `import { Hono } from 'hono'`)
  imports.add('cors', `import { cors } from 'hono/cors'`)
  imports.add('logger', `import { logger } from 'hono/logger'`)
  imports.add('serve', `import { serve } from '@hono/node-server'`)
  ...
  for (const [, model] of models) {
    parts.push(generateHonoData(model))
    parts.push('')
  }
  ...
  parts.push('app.onError((err, c) => {')
  parts.push('  console.error("Error:", err.message)')
  parts.push('  return c.json({ error: err.message }, 500)')
  parts.push('})')
  parts.push('')
  ...
  parts.push(`serve({ port: PORT, fetch: app.fetch }, (info) => {`)
  parts.push(`  console.log(\`Server running on port \${info.port}\`)`)
  parts.push('})')
  parts.push('')
...
function generateHonoData(model: DataModel): string {
  const lines: string[] = []
...
function generateRoute(
  ...
  lines.push(`app.${method}('${fullPath}', async (c) => {`)
  lines.push(`  try {`)
  ...
  if (method === 'get') {
    ...
    lines.push(`    return c.json({ message: '${method.toUpperCase()} ${fullPath}' })`)
  }

  lines.push(`  } catch (err: any) {`)
  lines.push(`    console.error(err)`)
  lines.push(`    return c.json({ error: err.message }, 500)`)
  lines.push(`  }`)
  lines.push('})')
...
function generateListHandler(
  ...
  if (filterProp) {
    ...
    for (const field of fields) {
      lines.push(`    if (c.req.query('${field}')) result = result.filter(i => String(i['${field}']) === c.req.query('${field}'))`)
    }
  }
  ...
  if (paginateProp) {
    ...
    lines.push(`    const page = parseInt(c.req.query('page') || '1')`)
...
function generateGetByIdHandler(
  ...
  lines.push(`    if (!item) {`)
  lines.push(`      return c.json({ error: '${errMsg}' }, ${errCode})`)
  lines.push(`    }`)
...
function generateCreateHandler(
  ...
  if (fields.length > 0) {
    lines.push(`    const { ${fields.join(', ')} } = body`)
  }
  ...
  if (validateProp && ruleNames.length > 0) {
    ...
    for (const ruleName of ruleNames) {
      lines.push(`    validationErrors.push(...validate${ruleName}(body))`)
    }
    lines.push(`    if (validationErrors.length > 0) {`)
    lines.push(`      return c.json({ errors: validationErrors }, 400)`)
    lines.push(`    }`)
  }
  ...
  lines.push(`    return c.json(item, ${statusCode})`)
}
...
function generateUpdateHandler(
  ...
  if (acceptProp) {
    ...
    for (const field of fields) {
      const cleanName = field.replace(/,/g, '').trim()
      lines.push(`    if (body['${cleanName}'] !== undefined) updates['${cleanName}'] = body['${cleanName}']`)
    }
    for (const sp of setProps) {
      const parsed = parseSetValue(sp.args.join(' '))
      if (parsed) {
    ...
    lines.push(`    ${dataModel.storeName}[index] = { ...${dataModel.storeName}[index], ...body, id }`)
  }

  const statusRaw = statusProp ? statusProp.args.join(' ') : ''
  const statusCode = extractStatusOrError(statusRaw, statusProp, 200)
  lines.push(`    return c.json(${dataModel.storeName}[index], ${statusCode})`)
}
...
function generateDeleteHandler(
  ...
  if (statusCode === 204) {
    ...
    lines.push(`    return c.json(deleted, ${statusCode})`)
  }
}
