...
export function generateKoaCode(ast: ClearFile): string {
  const parts: string[] = []

  parts.push(`// Auto-generated by Clear v0.2 — Koa`)
  parts.push(`// Source: ${ast.product.name}`)
  parts.push(`// Do not edit — changes should be made to the .clear file`)
  parts.push('')

  const imports = new ImportMap()
  imports.add('koa', `import Koa from 'koa'`)
  imports.add('router', `import Router from '@koa/router'`)
  imports.add('cors', `import cors from '@koa/cors'`)
  imports.add('bodyParser', `import { bodyParser } from '@koa/bodyparser'`)
  ...
  for (const [, model] of models) {
    parts.push(generateKoaData(model))
    parts.push('')
  }
  ...
  parts.push('const app = new Koa()')
  parts.push('const router = new Router()')
  parts.push('')
  parts.push('app.use(cors())')
  parts.push('app.use(bodyParser())')
  parts.push('')
  ...
  // Register routes
  parts.push('// Register routes')
  parts.push('app.use(router.routes()).use(router.allowedMethods())')
  parts.push('')

  // Error handling
  parts.push('// Error handling')
  parts.push('app.on("error", (err, ctx) => {')
  parts.push('  console.error("Error:", err.message)')
  parts.push('  ctx.status = err.status || 500')
  parts.push('  ctx.body = { error: err.message }')
  parts.push('})')
  parts.push('')
...
function generateKoaData(model: DataModel): string {
  const lines: string[] = []
...
function generateRoute(
  ...
  lines.push(`router.${method}('${fullPath}', async (ctx) => {`)
  lines.push(`  try {`)
  ...
  if (method === 'get') {
    ...
    lines.push(`    ctx.body = { message: '${method.toUpperCase()} ${fullPath}' }`)
  }

  lines.push(`  } catch (err: any) {`)
  lines.push(`    console.error(err)`)
  lines.push(`    ctx.status = 500`)
  lines.push(`    ctx.body = { error: err.message }`)
  lines.push(`  }`)
  lines.push('})')
...
function generateListHandler(
  ...
  if (filterProp) {
    ...
    for (const field of fields) {
      lines.push(`    if (ctx.query['${field}']) result = result.filter(i => String(i['${field}']) === ctx.query['${field}'])`)
    }
  }
  ...
  if (paginateProp) {
    ...
    lines.push(`    const page = parseInt((ctx.query as any).page || '1')`)
...
function generateGetByIdHandler(
  ...
  lines.push(`    const id = ctx.params.id`)
  lines.push(`    const item = ${dataModel.storeName}.find(i => i.id === id)`)
  lines.push(`    if (!item) {`)
  lines.push(`      ctx.status = ${errCode}`)
  lines.push(`      ctx.body = { error: '${errMsg}' }`)
  lines.push(`      return`)
  lines.push(`    }`)
...
function generateCreateHandler(
  ...
  if (fields.length > 0) {
    lines.push(`    const { ${fields.join(', ')} } = ctx.request.body as any`)
  } else {
    lines.push(`    const data = ctx.request.body as any`)
  }
  ...
  if (validateProp && ruleNames.length > 0) {
    ...
    for (const ruleName of ruleNames) {
      lines.push(`    validationErrors.push(...validate${ruleName}(ctx.request.body as any))`)
    }
    lines.push(`    if (validationErrors.length > 0) {`)
    lines.push(`      ctx.status = 400`)
    lines.push(`      ctx.body = { errors: validationErrors }`)
    lines.push(`      return`)
    lines.push(`    }`)
  }
  ...
  lines.push(`    ctx.status = ${statusCode}`)
  lines.push(`    ctx.body = item`)
}
...
function generateUpdateHandler(
  ...
  if (acceptProp) {
    ...
    for (const field of fields) {
      const cleanName = field.replace(/,/g, '').trim()
      lines.push(`    if ((ctx.request.body as any)['${cleanName}'] !== undefined) updates['${cleanName}'] = (ctx.request.body as any)['${cleanName}']`)
    }
    for (const sp of setProps) {
      const parsed = parseSetValue(sp.args.join(' '))
      if (parsed) {
    ...
    lines.push(`    ${dataModel.storeName}[index] = { ...${dataModel.storeName}[index], ...(ctx.request.body as any), id }`)
  }

  const statusRaw = statusProp ? statusProp.args.join(' ') : ''
  const statusCode = extractStatusOrError(statusRaw, statusProp, 200)
  lines.push(`    ctx.status = ${statusCode}`)
  lines.push(`    ctx.body = ${dataModel.storeName}[index]`)
}
...
function generateDeleteHandler(
  ...
  if (statusCode === 204) {
    lines.push(`    ctx.status = 204`)
    lines.push(`    ctx.body = null`)
  } else {
    lines.push(`    ctx.status = ${statusCode}`)
    lines.push(`    ctx.body = deleted`)
  }
}
